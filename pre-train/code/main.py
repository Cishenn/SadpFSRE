...
def train(args, model, train_dataset):
    ...
    params = {"batch_size": args.batch_size_per_gpu, "shuffle": True, "drop_last": True}
    ...
    for i in range(args.max_epoch):
        for step, batch in enumerate(train_dataloader):
            inputs = {"input": batch[0].to(args.device), "mask": batch[1].to(args.device), "rel_input": batch[2].to(args.device), "rel_mask": batch[3].to(args.device), "h_pos": batch[4].to(args.device), 't_pos': batch[5].to(args.device)}
            model.train()
            ...
            if step % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [0, -1] and global_step % args.save_step == 0:
                    ...
                    ckpt_bert2 = {
                        'bert-base': model.model2.bert.state_dict(),
                    }
                    torch.save(ckpt_bert1, os.path.join("../ckpt/ckpt_SadpFSRE_bert1", "ckpt_of_step_"+str(global_step)))
                    torch.save(ckpt_bert2, os.path.join("../ckpt/ckpt_SadpFSRE_bert2", "ckpt_of_step_"+str(global_step)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--temperature", dest="temperature", type=float,
                        default=0.05, help="temperature for contrastive loss")
    ...
    # Model and dataset
    if args.model == "SadpFSRE":
        model = SadpFSRE(args).to(args.device)
        train_dataset = SadpFSREDataset("../data/SadpFSRE", args)
    else:
        raise Exception("No such model! Please make sure that `model` takes the value in {SadpFSRE}")
